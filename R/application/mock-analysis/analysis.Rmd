...
In this report, we analyze the synthetic data that resemble the data in the case
...
The synthetic data are read via the SAS data file. These data are in a
...
# ADAS-Cog score at baseline. We transform the data into the long format.
data_lighthouse_long = data_lighthouse %>%
  # The SAS-data set contains the change scores as outcome variable, instead of
  # the ADAS-Cog score itself. Add baseline value to the change score to get the
  # original ADAS-Cog score.
...
# estimating the acceleration factors, we require that the mean response at
# baseline is equal in both treatment groups. This can be "forced" by setting
...
In the second step, we use `TCT_meta()` to estimate the time-specific
acceleration factors; the estimated acceleration factors are shown below. Note
that the z-values for the estimated acceleration factors are equal to the
corresponding t-values in the EMM contrasts. This is no coincidence and always
holds for the `"score"` tests for the time-specific acceleration factors. The
p-values below and the p-values for the EMM contrasts differ slightly because
`TCT_meta()` uses the standard normal distribution instead of the t-distribution
