...
data_lighthouse = haven::read_sas("application/effect.sas7bdat") %>%
  rename(WEEK = time, SUBJID = id, TRT = trt) %>%
  mutate(WEEK = case_match(WEEK, 1 ~ 12, 2 ~ 24, 3 ~ 40, 4 ~ 48),
         TRT = case_match(TRT, 0~"Placebo", 1~"Active"),
         SITE = factor(SITE))
# The original data are not in the long format because there is no separate row
# for the ADASCog at baseline. We recode the data into the long format.
data_lighthouse_long = data_lighthouse %>%
  # Add baseline value to the change score to get the original ADASCog score.
...
A MMRM leads to valid inferences under MAR regardless of the proportion of
missing values; however, the results could be sensitive to small violations of
MAR because there are many missing values.
