...
[^2]: 13-item cognitive subscale of the Alzheimer’s disease assessment scale, 
...
results_tbl_estimation = results_tbl %>%
  mutate(se_log = (1 / estimate ** 2) * se_TCT_common) %>%
...
results_tbl_estimation %>%
  filter(constraints == FALSE, inference == "least-squares") %>%
...
results_tbl_estimation %>%
  ...
  scale_x_continuous(trans = "log10", name = "Sample Size, n") +
  scale_linetype(name = "Progression Rate") +
  facet_grid(gamma_slowing ~ drop_first_occasions)
```
...
```{r}
results_tbl_estimation %>%
  filter(constraints == FALSE, inference == "least-squares", progression == "normal",
...
```{r}
results_tbl_inference %>%
  filter(constraints == FALSE, inference == "least-squares", drop_first_occasions == 0) %>%
  ggplot(aes(
    x = n,
    y = coverage_bs,
    color = `Follow Up`,
  ...
  facet_grid(rows = "gamma_slowing", scales = "free")
```

Add plot with type 1 error and power

```{r}
results_tbl_inference %>%
  filter(constraints == FALSE, inference == "least-squares", drop_first_occasions == 0, progression == "normal") %>%
  ggplot(aes(
    x = n,
    y = rejection_rate_bs,
    color = `Follow Up`,
