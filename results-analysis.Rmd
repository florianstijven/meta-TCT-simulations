...
replications throughout. Also, we also only consider the bootstrap for the
...
-   **Bias**
    ...
    -   The "wrong" interpolation method can lead to a small bias, even
        in large samples. The results show that, when the data are
        generated with cubic spline interpolation, but the meta TCT
        estimator uses linear interpolation, there can be a small amount
        of bias that that does not disappear with an increasing sample
        size.
